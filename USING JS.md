...
❗️`if(!cur) return [0, 0]` 这里要注意，一定得返回00
...
**1、淘汰分数**  [淘汰分数](https://www.nowcoder.com/exam/test/67505335/detail?pid=28665343&examPageSource=Company&testCallback=https%3A%2F%2Fwww.nowcoder.com%2Fexam%2Fcompany%3FcurrentTab%3Drecommand%26jobId%3D100%26selectStatus%3D0%26tagIds%3D179,644&testclass=%E8%BD%AF%E4%BB%B6%E5%BC%80%E5%8F%91)
...
void (async function () {
    ...
    let left = parseInt(info[0][1])
    let right = parseInt(info[0][2])
    let num = parseInt(info[0][0])

    for (let i = left; i <= right; i++) {
        if (scores[i] !== scores[i - 1]) {
            if(left<=num-i && num-i<=right) {
                res = parseInt(scores[i-1]);
                break
            }
        }
    }
...
**2、正则序列** [正则序列](https://www.nowcoder.com/exam/test/67505335/detail?pid=28665343&examPageSource=Company&testCallback=https%3A%2F%2Fwww.nowcoder.com%2Fexam%2Fcompany%3FcurrentTab%3Drecommand%26jobId%3D100%26selectStatus%3D0%26tagIds%3D179,644&testclass=%E8%BD%AF%E4%BB%B6%E5%BC%80%E5%8F%91)
...
void async function () {
    ...
    for(let i=0; i<num; i++) {
        res += Math.abs(parseInt(nums[i])-i-1)
    }
    console.log(res)
}()
```
...
火车迷  [剑指Offer31. 栈的压入、弹出序列](https://leetcode.cn/problems/zhan-de-ya-ru-dan-chu-xu-lie-lcof/)
...
这里考虑最多能装多少块巧克力板，所以用前缀和

再在构造出来的前缀和数组中二分查找



确实 想不到前缀和&二分 dp做的也不会验证😭
